...
#                                 'phase_correlation':100}
    ...
    def fill_random_weights(self):
        if self.category_weights['mean'] == 0:
            self.category_weights['mean'] = random.randint(0, 10)
        if self.category_weights['bandpass_variance'] == 0:
    ...
    def save_weights_to_file(self):
        with open(self.data_file, 'w') as f:
            values = [
                self.category_weights['mean'],
                self.category_weights['bandpass_variance'],
                self.category_weights['edge_mean'],
                self.category_weights['phase_correlation']
            ]
    ...
    def read_weights_from_file(self):
        with open(self.data_file, 'r') as f:
            ...
            self.category_weights['mean'] = values[0]
            self.category_weights['bandpass_variance'] = values[1]
            self.category_weights['edge_mean'] = values[2]
            self.category_weights['phase_correlation'] = values[3]
...
#        self.stat_high_variance_to_zero = False       # Not actually a statistic, but option to try to force high-pass to zero in metamer
        ...
        self.named_stat_groups = { 
                 ...
                 'ps_all':('ps_marginalstatistics',' ','ps_magnitudecorrelation','ps_crossscalephase'),
