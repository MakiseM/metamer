...
def rename_and_delete_weights_file(target_file_path, weights_file_path):
    # Step 1: Read the string from weights.txt
    with open(weights_file_path, 'r') as f:
        new_name = f.read().strip()

    # Step 2: Rename the target file
    new_file_path = os.path.join(os.path.dirname(target_file_path), new_name + '.png')
    os.rename(target_file_path, new_file_path)
    print(f"Renamed '{target_file_path}' to '{new_file_path}'")

    # Step 3: Delete the weights.txt file
    os.remove(weights_file_path)
    print(f"Deleted '{weights_file_path}'")
...
FS_gaze_pool = '128:Kern=Trig:mesa=1/2:stride=1/2:Bound=wrap_x'
...
gaze_over4_pool = '128:Kern=Trig:mesa=1/2:stride=1/4:Bound=wrap_x'
# Pooling setting for gaze-centric metamer with medium overlap (half-region spacing between region centers)
gaze_over2_pool = '128:Kern=Trig:mesa=0:stride=1/2:Bound=wrap_x'
...
#    MetamerConfig('_original',copy_original=True, pooling=gaze_over4_pool),
    MetamerConfig('_gazemet', pooling=gaze_over4_pool, pyramid=better_gaze_pyr, warping=FS_gaze_warp,stats='Metamer'),
    )
...
generate_image_schedule('../sampleimages/shashi.jpg', OurGazeWarpSched, color=True, max_iters=iters, basename='good1')
generate_image_schedule('../sampleimages/bigben.jpg', OurGazeWarpSched, color=True, max_iters=iters, basename='good2')
generate_image_schedule('../sampleimages/EIN.jpg', OurGazeWarpSched, color=True, max_iters=iters, basename='good3')
generate_image_schedule('../sampleimages/buffon.png', OurGazeWarpSched, color=True, max_iters=iters, basename='good4')